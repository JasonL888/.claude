...
**When equal-division doesn't match the audio** (e.g., there is an intro sentence before item 0, or items vary in length), add `cue_times` to the scene — an array of seconds from scene start at which each item becomes active:

```json
"cue_times": [0, 8.5, 14.5, 19.5]
```

Components fall back to equal-division when `cue_times` is absent. Tune values after audio is generated by scrubbing in Remotion Studio.
